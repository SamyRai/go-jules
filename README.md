...
The SDK is intentionally a single importable package. It does not include a
CLI, config loader, patch applier, artifact writer, GitHub client, or local
filesystem side effects. Application orchestration belongs in callers.
...
The module is pre-1.0, but current consumers are treated as real. Releases should
avoid breaking public API changes unless there is a documented API correction
that cannot be handled additively.

Versioning follows Go module conventions:

- patch releases fix bugs without public API changes
- minor releases may add backward-compatible API surface
...
- Introduce resource-service ownership through `client.Sessions`,
  `client.Sources`, `client.Activities`, and `client.Artifacts`; remove the old
  root-level resource methods.
...
go vet ./...
GOTOOLCHAIN=go1.26.3 go run golang.org/x/vuln/cmd/govulncheck@latest ./...
GOPROXY=proxy.golang.org go list -m github.com/SamyRai/go-jules@vX.Y.Z
```
