...
func TestContractFixtures(t *testing.T) {
	t.Run("session", func(t *testing.T) {
		...
		assert.True(t, session.Archived)
		...
		assert.Equal(t, "jules/add-auth-tests", session.SourceContext.WorkingBranch)
		assert.True(t, session.SourceContext.EnvironmentVariablesEnabled)
		require.Len(t, session.Outputs, 2)
		require.NotNil(t, session.Outputs[0].PullRequest)
		assert.Equal(t, "main", session.Outputs[0].PullRequest.BaseRef)
		assert.Equal(t, "jules/add-auth-tests", session.Outputs[0].PullRequest.HeadRef)
		require.NotNil(t, session.Outputs[1].ChangeSet)
		require.NotNil(t, session.Outputs[1].ChangeSet.GitPatch)
	})
